			...
			while (gameInst->size() >= 2) {
				try {
					...
					if (gameInst->allFold != false) {
						...
						gameInst->around = false;
						gameInst->checks = 0;
						cout << "Please enter a game name if want to join new one (FiveCardDraw, SevenCardStud) or 'quit' to exit the program." << endl;
						string input;
						cin >> input;
						if (input == "quit") {
							play = false;
